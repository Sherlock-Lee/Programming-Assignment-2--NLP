...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    n = prob.shape[0]
    d = predicted.shape[0]
    gradPred = (prob.reshape((1, n)).dot(outputVectors)).flatten()
    grad = prob.reshape((n, 1)) * predicted.reshape((1, d))
    #raise NotImplementedError
    ### END YOUR CODE
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    a = sigmoid(np.dot(predicted.reshape(-1), outputVectors[target].T))
    cost += -np.log(a)
    grad[target:target + 1] = (a - 1) * predicted
    gradPred += (a - 1) * outputVectors[target]
    ...
    for i in range(K):
        ...
        neg = sigmoid(-np.dot(predicted.reshape(-1), outputVectors[j].T))
        cost += -np.log(neg)
        grad[j:j + 1] = (1 - neg) * predicted
        gradPred += (1 - neg) * outputVectors[j]
    #raise NotImplementedError
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for cwd in contextWords:
        idx = tokens[cwd]
        a, b, c = word2vecCostAndGradient(predicted, idx, outputVectors, dataset)
        cost += a
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    cost, b, gradOut = word2vecCostAndGradient(predicted, tokens[currentWord], outputVectors, dataset)
    gradIn = np.zeros(inputVectors.shape)
    for idx in indices:
        gradIn[idx, :] += b
    #raise NotImplementedError
